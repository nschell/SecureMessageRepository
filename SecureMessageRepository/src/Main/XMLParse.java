...
public class XMLParse {
    ...
    public static void SecureobjectToXML(AccessControl s, String filePath) throws FileNotFoundException {
        try {
            JAXBContext context = JAXBContext.newInstance(SecureDocument.class);
            Marshaller m = context.createMarshaller();
            m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            QName qName = new QName("main.java", "s");
            JAXBElement<AccessControl> root = new JAXBElement<AccessControl>(qName, AccessControl.class, s);
            m.marshal(s, new FileOutputStream(filePath));
    ...
    public static AccessControl SecureXMLToObject(String filePath) {
        try {
            ...
            Unmarshaller un = context.createUnmarshaller();
            AccessControl s = (AccessControl) un.unmarshal(new File(filePath));
            return s;
        } catch (JAXBException e) {
            e.printStackTrace();
        }
